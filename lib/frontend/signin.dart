  ...
  signinUser() async {
    if (formkey.currentState.validate()) {
      ...
      await authService
        ...
        if (val != null) {
          snapshot =
              await database.getuseremailinfo(emailTextEditingController.text);
          SPFunctions.saveUserLoggedInSharedPreference(true);
          SPFunctions.saveUserNameSharedPreference(
              snapshot.documents[0].data["name"]);
          SPFunctions.saveUserEmailSharedPreference(
    ...
    await authService.signInWithGoogle(context).then((val) async {
      ...
      if (val != null) {
        final QuerySnapshot result = await Firestore.instance
            .collection('users')
            .where('name', isEqualTo: val.displayName)
            .getDocuments();
        final List<DocumentSnapshot> documents = result.documents;
        final QuerySnapshot result1 = await Firestore.instance
            .collection('users')
            .where('email', isEqualTo: val.email)
            .getDocuments();
